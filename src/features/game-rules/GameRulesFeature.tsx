...
export function GameRulesFeature({ gameId }: { gameId: GameId }) {
  ...
  return <>
    ...
    {FLOATING.OPEN_KEYS.map(KEY => <FloatingPanel key={KEY} panelKey={KEY} title={KEY === 'score' ? '📖 점수 규칙' : '🏴‍☠️ 해적별 능력'} content={KEY === 'score' ? SCORE_CONTENT : PIRATE_CONTENT} position={FLOATING.POSITIONS[KEY]} paired={FLOATING.PAIRED} focused={FLOATING.PAIRED ? FLOATING.FOCUSED_KEY !== null : FLOATING.FOCUSED_KEY === KEY} onFocus={() => FLOATING.show(KEY)} onDrag={FLOATING.beginDrag} onHide={FLOATING.hide} />)}
  </>;
}
...
function FloatingPanel({ panelKey, title, content, position, paired, focused, onFocus, onDrag, onHide }: { panelKey: FloatingKey; title: string; content: ReactNode; position: { x: number; y: number }; paired: boolean; focused: boolean; onFocus: () => void; onDrag: (key: FloatingKey, event: React.PointerEvent<HTMLElement>) => void; onHide: (key: FloatingKey) => void }) {
  const [EXPANDED, SET_EXPANDED] = useState(true);
  return <aside data-floating-panel={panelKey} className={`floating-panel ${paired ? 'paired' : ''} ${focused ? 'focused' : ''}`} style={{ display: 'block', left: position.x, top: position.y }} onPointerDown={onFocus} onFocusCapture={onFocus}><div className="floating-header" onPointerDown={event => onDrag(panelKey, event)}><strong>{title}</strong><div className="floating-actions"><button onPointerDown={event => event.stopPropagation()} onClick={() => SET_EXPANDED(CURRENT => !CURRENT)}>{EXPANDED ? '접기' : '펼치기'}</button><button onPointerDown={event => event.stopPropagation()} onClick={() => onHide(panelKey)}>숨기기</button></div></div>{EXPANDED && <div className="floating-body">{content}</div>}</aside>;
}
