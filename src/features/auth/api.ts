...
export const internalEmail = (value: string) => `${normalizeId(value)}@${import.meta.env.VITE_AUTH_DOMAIN || 'danbi.playground.com'}`;
...
export async function signIn(input: { id: string; password: string }) {
  ...
  if (error) throw error;
  localStorage.removeItem('board-game-guest-session-v1');
}
