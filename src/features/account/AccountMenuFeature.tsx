...
export function AccountMenuFeature({ profile, guest }: { profile: Profile | null; guest: boolean }) {
  ...
  return <>
    ...
    <nav className={`side-menu ${OPEN ? 'open' : ''}`}><div className="side-menu-header">메뉴</div>
      ...
      <button className={`side-menu-item ${LOCATION.pathname === '/' ? 'current' : ''}`} onClick={() => GO('/')}>친구 목록</button>
      <button className={`side-menu-item ${LOCATION.pathname === '/history' ? 'current' : ''}`} onClick={() => GO('/history')}>게임 기록</button>
      <button className={`side-menu-item ${LOCATION.pathname === '/ranking' ? 'current' : ''}`} onClick={() => GO('/ranking')}>랭킹</button>
      <div className="side-menu-divider"/><button className={`side-menu-item ${LOCATION.pathname === '/games' ? 'current' : ''}`} onClick={() => GO('/games')}>게임 점수 기록</button>
    </nav>
