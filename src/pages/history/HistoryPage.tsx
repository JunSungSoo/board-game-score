...
export function HistoryPage() {
  ...
  return <><AccountMenuFeature profile={PROFILE} guest={GUEST}/><div className="container account-page-container"><header className="account-page-header"><span className={`account-status ${PROFILE ? 'signed-in' : ''}`}>{PROFILE?.display_name ?? '게스트'}</span><h1>게임 기록</h1><p>완료한 게임과 점수, 순위를 확인해요</p></header><GameRecordCtaFeature/><AccountRouteNavFeature/><GameHistoryFeature profile={PROFILE}/></div></>;
}
