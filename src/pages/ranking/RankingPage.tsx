...
export function RankingPage() {
  ...
  return <><AccountMenuFeature profile={PROFILE} guest={GUEST}/><div className="container account-page-container"><header className="account-page-header"><span className={`account-status ${PROFILE ? 'signed-in' : ''}`}>{PROFILE?.display_name ?? '게스트'}</span><h1>랭킹</h1><p>1등을 가장 많이 기록한 순서예요</p></header><GameRecordCtaFeature/><AccountRouteNavFeature/><RankingFeature profile={PROFILE}/></div></>;
}
