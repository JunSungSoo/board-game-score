...
export function GamePage() {
  ...
  function chooseMode(mode: GameMode) { SET_MODE(mode); if (mode === 'me') start({ names: ['나'], membersA: [], membersB: [] }, mode); else SET_STAGE('setup'); }
  function start(setup: SetupResult, forcedMode = MODE) {
    ...
    else if (GAME_ID === 'tichu') NEXT = { gameId: GAME_ID, mode: 'all', round: 1, tichuMode: 'zheng', players: setup.names.map(MAKE_ENTITY) };
  ...
  return <><div className="game-bg" data-active={BACKGROUND_GAME ?? undefined}/><AccountMenuFeature profile={PROFILE} guest={GUEST} onGameSelect={() => { CLEAR_GAME(); SET_STAGE('select'); }} />
    <div className="container"><header><button className={`account-status ${PROFILE ? 'signed-in' : ''}`}>{PROFILE?.display_name ?? '게스트'}</button><h1>🎲 보드게임 점수 계산기</h1><p>보드게임 점수를 쉽고 정확하게</p></header>
      ...
      {STAGE === 'setup' && <PlayerSetupFeature guestOnly={CHOICE === 'generic'} team={CHOICE === 'generic' || CHOICE === 'tichu-team'} min={CHOICE === 'tichu-zheng' ? 3 : 2} max={CHOICE === 'generic' ? 20 : CHOICE === 'tichu-zheng' ? 6 : 8} onStart={start} onBack={() => SET_STAGE(CHOICE.startsWith('tichu') ? 'select' : 'mode')} />}
